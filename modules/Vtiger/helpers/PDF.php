...
class PDF_helper {
	...
	public static function createPDF($templateId, $recordIdData, $moduleName, $isMassExport = false, $is_preview = false)
	{
		...
		if(!$is_headlesschrome){
			...
			$pdfdataarray = array();
			// shell_exec("/usr/bin/google-chrome --headless --no-sandbox --disable-setuid-sandbox --disable-software-rasterizer --disable-gpu --virtual-time-budget=9999999 --run-all-compositor-stages-before-draw --disable-dev-shm-usage --no-pdf-header-footer --print-to-pdf=" . $filepath . ".pdf" . " " . $filepath . ".html");
			...
			foreach ($recordIdData as $key => $recordId) {
				...
				if(empty($dokerfiledirectory)) {
					$command = $chromeurl." --headless --disable-gpu --disable-dev-shm-usage --no-pdf-header-footer --print-to-pdf=" . $hostfilepath . ".pdf " . $hostfilepath . ".html";
					exec($command);
				} else {
