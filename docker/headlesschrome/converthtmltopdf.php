...
if ($filepath) {
    ...
    for ($i = 0; $i < $max_loop_count; $i++) {
        // htmlからPDF変換
        exec("/usr/bin/google-chrome --headless --no-sandbox --disable-setuid-sandbox --disable-software-rasterizer --disable-gpu --allow-file-access-from-files --virtual-time-budget=9999999 --run-all-compositor-stages-before-draw --disable-dev-shm-usage --no-pdf-header-footer --print-to-pdf=" . $filepath . ".pdf" . " " . $filepath . ".html 2>&1", $output, $retval);
        if ($retval != 0) {
        ...
        if (file_exists($filepath) && filesize($filepath . ".pdf") > 10240) {
            break;
        }
